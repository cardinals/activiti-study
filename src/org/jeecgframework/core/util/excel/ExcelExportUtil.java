...
import org.jeecgframework.core.annotation.excel.Dictionary;
...
import org.jeecgframework.core.annotation.excel.SingleObjectEntity;
...
public class ExcelExportUtil
{
		...
		for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
			...
			if (entity.getList() != null) {
				...
				Object value = entity.getGetMethods() != null ? getFieldBySomeMethod(
						...
						.invoke(t, new Object[] {});
				// --add by zhu
				if (StringUtil.isNotEmpty(entity.getDictionary())) {
					value = processDictionary(entity, value);
				}
				// --end by zhu
	...
	private static Object processDictionary(ExcelExportEntity entity,
			Object value) {
		// 处理dictionary映射
		ApplicationContext ac = WebApplicationContextUtils
				.getRequiredWebApplicationContext(context);
		SystemService systemService = ac.getBean(SystemService.class);
		String sql = "select t.typename from t_s_type t,t_s_typegroup g where t.typegroupid=g.id and g.typegroupcode=? and t.typecode=?";

		Map<String, Object> map = systemService.findOneForJdbc(sql,
				entity.getDictionary(), value);
	...
	private static void createListCells(Drawing patriarch, int index,
			...
			Sheet sheet, HSSFWorkbook workbook) throws Exception {
		ExcelExportEntity entity;
		Row row;
	 ...
	 * @param index
	 */
	private static void createTitleRow(ExcelTitle title, Sheet sheet,
	...
	private static void createImageCell(Drawing patriarch,
			ExcelExportEntity entity, Row row, int i, String string) {
		...
		try {
			String path = ExcelExportUtil.class.getClassLoader()
					.getResource("") + string;
	...
	private static void getAllExcelField(String targetId, Field[] fields,
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (isCollection(field.getType())) {
			...
			else if (isUseSingleObject(field, targetId)) {
				// 如果配置了SingleObjectEntity表示只导出对应的字段，不做全部导出
				List<Method> newMethods = new ArrayList<Method>();
				if (getMethods != null) {
					newMethods.addAll(getMethods);
				}
				newMethods.add(getMethod(field.getName(), pojoClass));
				getAllExcelField(targetId, getClassFieldsFilter(field),
				...
				newMethods.add(getMethod(field.getName(), pojoClass));
				getAllExcelField(targetId, getClassFields(field.getType()),
						excelParams, field.getType(), newMethods);
			}
		}
	}
	...
	private static boolean isUseSingleObject(Field field, String targetId) {
		boolean boo = false;

		if (field.getAnnotation(SingleObjectEntity.class) != null
				&& isUseInThis(field.getAnnotation(SingleObjectEntity.class)
						.exportField(), targetId)) {
			boo = true;
		}
	...
	private static void getExcelFieldList(String targetId, Field[] fields,
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (isJavaClass(field)) {
				...
				newMethods.add(getMethod(field.getName(), pojoClass));
				getExcelFieldList(targetId, getClassFields(field.getType()),
	...
	private static boolean isCollection(Class<?> clazz) {
		...
		for (Class<?> face : faces) {
			if (face.getName().equals(colleciton)) {
				...
				if (face.getSuperclass() != Object.class
						&& face.getSuperclass() != null) {
	...
	/**
	 * 
	 * 通过SingleObjectEntity过滤不需要导出的字段
	 * 
	 * @param type
	 * @return
	 * @return:Field[]
	 ...
	 * @since 1.0.0
	 */
	private static Field[] getClassFieldsFilter(Field field) {
		List<Field> list = new ArrayList<Field>();
		SingleObjectEntity entity = field
				.getAnnotation(SingleObjectEntity.class);
		String exportField = entity.exportField();
		...
		do {
			fields = clazz.getDeclaredFields();
			for (int i = 0; i < fields.length; i++) {
				if (exportField.contains(fields[i].getName())) {
					length++;
					list.add(fields[i]);
				}
			}
		...
		return list.toArray(new Field[length]);
